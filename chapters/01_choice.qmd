...
# Choice Models and Simulation {#sec-choice}
...
Read the last line slowly, because the rest of this book lives inside it. The indicator function $I[\cdot]$ equals 1 when the unobserved factors produce the outcome $y$ and equals 0 otherwise. The integral averages that indicator over all the values the unobserved factors might take, weighting by their density $f(\varepsilon)$. In words: *the probability of an outcome is the share of the unobserved-factor distribution that produces that outcome.* Every choice probability we compute in this book --- logit, probit, nested, mixed, hierarchical --- is an instance of @eq-int_indicator with a particular choice of $h$ and $f$.
...
The simulated value `r format(mean(I), digits=3)` approximates the exact value 0.25 and can be made closer by increasing the number of draws used in the simulation.
...
How good is the approximation? Since each $I^{(r)}$ is a Bernoulli random variable with success probability $p = 0.25$, the simulation average has standard error $\sqrt{p(1-p)/R}$, which for $R=1{,}000$ is about `r format(sqrt(.25*.75/1000), digits=2)`. Quadrupling the number of draws halves the standard error --- the familiar (and, when you are impatient, infuriating) square-root rate of Monte Carlo methods. We will return to the accuracy of simulated probabilities with much more care in @sec-msl, where it directly affects the quality of our parameter estimates.


## A Binary Logit Example {#sec-binary_logit}
...
where $\x$ is a vector of observable explanatory variables, $\bfbeta$ is a vector of parameters that through the functional form $\x'\bfbeta$ effectively serve as weights on the attributes, and $\varepsilon$ is a scalar collecting the value of everything relevant to the shopper but unobserved by the researcher. The utility of *not* buying is normalized to zero --- a harmless-seeming convention that we will scrutinize carefully in @sec-mnl (identification) and again in @sec-extensions (the outside option).

The piece the researcher can compute, $\x'\bfbeta$, is called the *representative utility* and gets its own letter, $V$; the model says total utility is representative utility plus noise, $U = V + \varepsilon$. The split matters: $V$ is where the parameters live, and $\varepsilon$ is what the integral in @eq-int_indicator averages over.

Notice that $h$ has grown an argument: once we specify utility, the parameters $\bfbeta$ enter the behavioral rule, so we write $h(\x, \bfbeta, \varepsilon)$ and condition the choice probability on $\bfbeta$ as well --- $p(y|\x,\bfbeta)$. The framework of @eq-int_indicator is unchanged; $\bfbeta$ simply rides along with $\x$. The shopper buys when buying is better than not buying, so the behavioral rule $h$ is:
...
This particular pairing of $h$ and $f$ is one of the special cases where the integral in @eq-int_indicator has a closed form. Let me show you why. Integrating @eq-bin_logistic_dist gives the logistic CDF, $F(\varepsilon) = 1/(1+e^{-\varepsilon})$ (differentiate it to confirm you get $f$ back). The shopper buys when $\x'\bfbeta + \varepsilon > 0$, i.e., when $\varepsilon > -\x'\bfbeta$, so
$$
p(y=1|\x,\bfbeta) = \Pr(\varepsilon > -\x'\bfbeta) = 1 - F(-\x'\bfbeta) = 1 - \frac{1}{1+e^{\x'\bfbeta}} = \frac{e^{\x'\bfbeta}}{1+e^{\x'\bfbeta}}
$$
No indicator, no integral sign left standing --- that is what "closed form" buys us:

$$
p(y=1 | \x, \bfbeta) = \frac{e^{\x'\bfbeta}}{1 + e^{\x'\bfbeta}}
$$ {#eq-binlogit_prob}

Let's make it concrete. Suppose the laptop on offer costs \$1,500 and has 16GB of RAM (an upgrade over a base 8GB configuration). We describe it with the vector $\x = (1, 1.5, 1)$: a constant, the price in thousands of dollars, and an indicator for the 16GB memory upgrade. And suppose the weights are $\bfbeta^\ast = (1.0, -1.2, 0.6)$ --- the asterisk is our standing notation for *true* parameter values, the ones the data generating process actually uses, as distinct from candidate values $\bfbeta$ we will later evaluate and estimates $\hat{\bfbeta}$ we will later compute. The three values: a baseline pull toward buying, a distaste for price, and a taste for memory.[^3] Then

$$
\x'\bfbeta = (1)(1.0) + (1.5)(-1.2) + (1)(0.6) = -0.2
$$

and the closed-form probability of purchase is

$$
p(y=1 | \x, \bfbeta) = \frac{e^{-0.2}}{1+e^{-0.2}} = `r format(exp(-0.2) / (1+exp(-0.2)), digits=3)`
$$ {#eq-binlogit_example}

[^3]: Only at this early stage --- exploring a fully specified model --- do we pick values for $\bfbeta$. In real research, $\x$ is data you collect and $\bfbeta$ contains parameters whose values you estimate. These particular values are not arbitrary: they are the *true parameters* of the running example, and we will spend most of this book trying to recover them from simulated data. Keep an eye on $-1.2$, the price coefficient; you will see it many times.

Even though we have the closed form, let's approximate the integral by simulation exactly as before, both to practice the procedure and to verify the closed form is what it claims to be. We use `rlogis()` to take $R=1{,}000$ draws from the standard logistic distribution, and we approximate the integral with the proportion of times $\x'\bfbeta + \varepsilon$ exceeds the threshold for action ($0$):
...
Our simulated value `r format(mean(U > 0), digits=3)` approximates the exact value `r format(exp(-0.2)/(1+exp(-0.2)), digits=3)`. The two routes --- analytic formula and simulation --- agree, as they must. Hold onto this dual view. When a closed form exists we will use it (it is faster and exact), but the simulation route never stops being available, and for most of the models in the second half of this book it is the *only* route.

### A model has implications

A specified model is a machine for answering "what if" questions, even before any data exist. What does our model say happens to the purchase probability as price varies? We can trace the whole curve by evaluating @eq-binlogit_prob over a grid of prices, for both memory configurations:

```{r}
#| label: fig-price-curve
#| fig-cap: "Purchase probability implied by the binary logit model as price varies, for the base (8GB) and upgraded (16GB) memory configurations. The point marks the worked example: a $1,500 laptop with 16GB of RAM."
#| fig-width: 6
#| fig-height: 3.5
...
## The Running Example: A Laptop Choice Study {#sec-running_example}
...
: Attributes of the laptop choice study. {#tbl-laptop-attributes}

This design is modest but honest: it mirrors the structure of real conjoint studies used throughout marketing practice [see @eggers2022, or the design chapters of @rao2014, for the craft of building such studies]. And it contains, in miniature, everything the models of this book need to exercise: a price for willingness-to-pay questions, brands for intercept-like effects (and, since Apple implies macOS while Dell and Acer imply Windows, a natural *nesting* of alternatives for @sec-iia), and attributes over which different respondents plausibly hold different tastes (the heterogeneity that drives everything from @sec-latentclass onward).

Because we will *simulate* this study rather than field it, we will always know the true preference parameters, and every estimation method we build will be judged by one standard: does it recover them? The binary examples of the next five chapters strip the setting down to a single laptop offer --- buy or don't --- carrying the true parameters $\bfbeta^\ast = (1.0, -1.2, 0.6)$ from @sec-binary_logit. In @sec-mnl the full study arrives: $N=500$ respondents, $T=8$ tasks each, $J=3$ laptops per task, and the complete attribute list of @tbl-laptop-attributes.

One warning as the example grows: it is *simulated*. Simulated data are the right tool for learning estimation --- the truth is known, so mistakes are visible --- but they are silent about the field problems of real conjoint studies (respondent attention, attribute framing, incentive alignment). When you graduate to real data, pair what you learn here with a treatment of those issues [e.g., @benakiva2019; @eggers2022].

## The Road Ahead

You now have the two ideas that generate this entire book:

1. **A discrete choice model is a pair $(h, f)$** --- a behavioral rule and a distribution for what we don't observe --- and the choice probability is the integral in @eq-int_indicator.
2. **Integrals can be simulated.** Draws from $f$ plus the rule $h$ approximate any choice probability, whether or not a closed form exists.

What we cannot yet do is the *reverse* inference. In this chapter we knew $\bfbeta$ and computed choice probabilities; research runs the other way, from observed choices back to unknown parameters. Getting there requires machinery: drawing from all the densities we'll need (@sec-draws), simulating entire datasets rather than single probabilities (@sec-simdata), organizing choice data so a computer can use them (@sec-data), turning data plus model into a likelihood (@sec-likelihood), and maximizing that likelihood numerically (@sec-optimization). That is Part I, and it is deliberately front-loaded: every hour spent there pays compound interest across Parts II through V.

## Key Learnings

- A discrete choice model is specified by a behavioral function $y = h(\x, \beps)$ and a density $f(\beps)$ for the unobserved factors. Specification and estimation are distinct tasks, and everything in this chapter was specification.
- Our focus is on the _probability_ of a choice outcome, $p(y|\x)$, which is an integral: the share of the unobserved-factor distribution that produces the outcome (@eq-int_indicator).
- That integral has a closed form only for special $(h, f)$ pairs --- the binary logit is one --- but it can be approximated by simulation for almost *any* pair: draw $\beps$ from $f$, apply $h$, average the indicator. Monte Carlo error shrinks at the $\sqrt{R}$ rate.
- A fully specified model has testable implications (S-curves, interaction-like gaps) before any data exist. Know what your specification commits you to.
- The running example is set: a laptop choice study with known true parameters, $\bfbeta^\ast = (1.0, -1.2, 0.6)$ in its binary form. The rest of the book is about recovering numbers like these from data.
